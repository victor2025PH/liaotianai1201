...
def kill_existing_processes():
    ...
    try:
        if sys.platform == 'win32':
            ...
            subprocess.run(["taskkill", "/F", "/IM", "node.exe", "/FI", "WINDOWTITLE eq *next*"], 
                         capture_output=True, timeout=5)
        else:
        ...
        with open(log_file, 'w', encoding='utf-8') as f:
            for line in iter(pipe.readline, ''):
                ...
                if any(keyword in line_lower for keyword in ['error', 'exception', 'failed', 'traceback', 'cannot', 'module']):
                    ...
                    log_queue.put(('warning', process_name, line))
    except Exception as e:
...
def start_frontend():
    ...
    if lock_file.exists():
        try:
            lock_file.unlink()
            print(f"[OK] 已清理锁文件: {lock_file}")
        except Exception as e:
            print(f"[WARNING] 清理锁文件失败: {e}")
...
def check_services():
    ...
    if frontend_ok:
        try:
            ...
            if "總覽儀表板" in content or "Dashboard" in content or "layout-wrapper" in content:
                print("[OK] 前端页面内容正常（包含应用界面）")
            elif "前端服務運行正常" in content:
                print("[WARNING] 前端显示简单状态页面，可能未加载应用内容")
                print("          可能原因：")
                print("          1. 认证检查中")
                print("          2. 需要登录")
                print("          3. 前端路由问题")
            else:
                print("[INFO] 前端页面内容需要进一步检查")
        except Exception as e:
            print(f"[WARNING] 检查前端页面内容时出错: {e}")
...
def signal_handler(sig, frame):
    ...
    if warnings:
        ...
        for warning in warnings[-5:]:
            print(f"[{warning['time']}] [{warning['service']}] {warning['warning']}")
