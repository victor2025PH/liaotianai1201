...
from app.api.group_ai import accounts, scripts, monitor, control, role_assignments, script_versions, servers, dashboard, alert_rules, dialogue, redpacket, script_review, role_assignment_schemes, export, automation_tasks, telegram_alerts, session_export, account_allocation, allocation, account_management, account_import, script_deployment, chat_features, advanced_features, private_chat_funnel, alert_management, ai_provider, keyword_monitor, keyword_monitor, keyword_triggers, scheduled_messages, group_management
...
router.include_router(
    account_import.router,
    prefix="/accounts",
    tags=["account-import"],
    # dependencies=protected_dependency,  # 移除路由级别依赖，使用端点级别依赖
)
router.include_router(
    script_deployment.router,
    prefix="/scripts",
...
# 高級聊天功能 API
router.include_router(
    chat_features.router,
    tags=["chat-features"],
    # dependencies=protected_dependency,
)

# 高級功能 API (TTS, 圖片, 跨群, 告警, 模板, 黑白名單, 多語言, Webhook)
router.include_router(
...
# 關鍵詞觸發規則 API
router.include_router(
    keyword_triggers.router,
    tags=["keyword-triggers"],
    # dependencies=protected_dependency,
)
