#!/bin/bash
# 彻底修复 - 重新构建standalone

cd /home/ubuntu/liaotian/saas-demo

echo "=== [1] 停止服务 ==="
sudo systemctl stop liaotian-frontend

echo ""
echo "=== [2] 清理旧的构建 ==="
rm -rf .next/standalone

echo ""
echo "=== [3] 重新构建 ==="
export NVM_DIR="$HOME/.nvm"
[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"
nvm use 20
npm run build

echo ""
echo "=== [4] 检查standalone目录结构 ==="
echo "standalone目录内容:"
ls -la .next/standalone/liaotian/saas-demo/ | head -15

echo ""
echo "=== [5] 检查.next目录（如果存在） ==="
ls -la .next/standalone/liaotian/saas-demo/.next/ 2>&1 | head -10 || echo ".next目录不存在（这是正常的）"

echo ""
echo "=== [6] 确保静态文件在正确位置 ==="
# 根据Dockerfile，static应该在standalone根目录的.next/static
if [ ! -d ".next/standalone/liaotian/saas-demo/.next/static" ]; then
    echo "创建.next/static目录并复制静态文件..."
    mkdir -p .next/standalone/liaotian/saas-demo/.next
    cp -r .next/static .next/standalone/liaotian/saas-demo/.next/
    echo "✅ 静态文件已复制"
fi

# 确保BUILD_ID存在
if [ ! -f ".next/standalone/liaotian/saas-demo/.next/BUILD_ID" ]; then
    if [ -f ".next/BUILD_ID" ]; then
        cp .next/BUILD_ID .next/standalone/liaotian/saas-demo/.next/
        echo "✅ BUILD_ID已复制"
    fi
fi

echo ""
echo "=== [7] 验证文件结构 ==="
echo "standalone目录:"
ls -la .next/standalone/liaotian/saas-demo/ | head -10
echo ""
echo ".next目录:"
ls -la .next/standalone/liaotian/saas-demo/.next/ 2>&1 | head -10

echo ""
echo "=== [8] 手动测试服务器启动 ==="
cd .next/standalone/liaotian/saas-demo
export NVM_DIR="$HOME/.nvm"
[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"
nvm use 20
echo "测试命令: timeout 5 node server.js"
timeout 5 node server.js 2>&1 || echo "测试完成"

echo ""
echo "=== [9] 重启服务 ==="
cd /home/ubuntu/liaotian/saas-demo
sudo systemctl start liaotian-frontend
sleep 5

echo ""
echo "=== [10] 检查服务状态 ==="
sudo systemctl status liaotian-frontend --no-pager | head -15

echo ""
echo "=== [11] 查看最新日志（如果有错误） ==="
if ! sudo systemctl is-active --quiet liaotian-frontend; then
    echo "服务未运行，查看错误日志:"
    sudo journalctl -u liaotian-frontend -n 10 --no-pager | tail -10
fi

echo ""
echo "=== [12] 测试静态文件访问 ==="
curl -I http://localhost:3000/_next/static/chunks/adc3be135379192a.js 2>&1 | head -5

echo ""
echo "=== 完成 ==="

