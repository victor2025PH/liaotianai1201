...
def main():
    ...
    try:
        ...
        # 1. 查找所有包含 aikz.usdt2026.cc 的配置文件
        print(f"\n{'='*60}")
        print("📋 查找冲突的 Nginx 配置")
        print(f"{'='*60}")
        
        stdout, stderr, exit_code = run_command(
            client,
            f"sudo grep -r 'server_name.*aikz' /etc/nginx/ 2>/dev/null | grep -v '^#'",
            "查找所有配置",
            check_output=False
        )
        ...
        stdout, stderr, exit_code = run_command(
            client,
            f"ls -la /etc/nginx/sites-enabled/ 2>/dev/null || echo 'sites-enabled 目录不存在'",
            "列出启用的站点",
            check_output=False
        )
        ...
        run_command(
            client,
            f"sudo cp -r /etc/nginx/sites-enabled /etc/nginx/sites-enabled.backup.$(date +%Y%m%d_%H%M%S) 2>/dev/null || true",
            "备份配置",
            check_output=False
        )
        
        # 4. 禁用所有冲突的配置（除了我们刚创建的）
        print(f"\n{'='*60}")
        print("📋 禁用冲突配置")
        print(f"{'='*60}")
        ...
        run_command(
            client,
            f"sudo ls /etc/nginx/sites-enabled/ | grep -v 'aikz.conf' | xargs -I {{}} sudo rm -f /etc/nginx/sites-enabled/{{}} 2>/dev/null || true",
            "禁用其他配置",
            check_output=False
        )
    ...
    location /redoc {
        ...
        # 写入配置
        print(f"\n{'='*60}")
        print("📋 写入正确的 Nginx 配置")
        print(f"{'='*60}")
        
        # 使用 heredoc 写入配置
        run_command(
            client,
            f"sudo bash -c \"cat > /etc/nginx/sites-available/aikz.conf << 'NGINXEOF'\n{nginx_config}\nNGINXEOF\"",
            "写入配置",
            check_output=False
        )
