# 分步部署指南

由於SSH密碼認證需要交互式輸入，我們將分步驟進行部署。請按照以下步驟執行：

## 步驟1: 測試SSH連接

在PowerShell中執行：

```powershell
# 清理舊的主機密鑰
ssh-keygen -R 165.154.254.99

# 測試連接（會提示輸入密碼: Along2025!!!）
ssh -o StrictHostKeyChecking=accept-new root@165.154.254.99 "echo '連接成功'"
```

## 步驟2: 執行基礎環境部署

我已經為您準備了一個遠程部署腳本。請執行：

```powershell
cd "E:\002-工作文件\重要程序\聊天AI群聊程序"
.\scripts\deployment\deploy_remote_interactive.ps1
```

**注意**: 當提示輸入密碼時，輸入: `Along2025!!!`

## 步驟3: 上傳項目文件

基礎環境部署完成後，上傳項目文件：

```powershell
# 方法1: 使用SCP上傳（需要輸入密碼）
scp -r group_ai_service root@165.154.254.99:/opt/group-ai/
scp -r admin-backend/app/api/group_ai root@165.154.254.99:/opt/group-ai/admin-backend/app/api/
scp config.py root@165.154.254.99:/opt/group-ai/

# 方法2: 打包後上傳（推薦）
cd "E:\002-工作文件\重要程序\聊天AI群聊程序"
tar czf group-ai-deploy.tar.gz group_ai_service/ admin-backend/app/api/group_ai/ config.py
scp group-ai-deploy.tar.gz root@165.154.254.99:/tmp/
ssh root@165.154.254.99 "cd /opt/group-ai && tar xzf /tmp/group-ai-deploy.tar.gz && rm /tmp/group-ai-deploy.tar.gz"
```

## 步驟4: 安裝Python依賴

```powershell
# 創建requirements.txt並上傳
ssh root@165.154.254.99 "cat > /opt/group-ai/requirements.txt << 'EOF'
pyrogram>=2.0.0
tgcrypto>=1.2.5
fastapi>=0.104.0
uvicorn[standard]>=0.24.0
pydantic>=2.0.0
pydantic-settings>=2.0.0
sqlalchemy>=2.0.0
alembic>=1.12.0
aiosqlite>=0.19.0
redis>=5.0.0
hiredis>=2.2.0
openai>=1.0.0
tiktoken>=0.5.0
python-dotenv>=1.0.0
pyyaml>=6.0
aiofiles>=23.2.0
httpx>=0.25.0
prometheus-client>=0.19.0
EOF"

# 安裝依賴
ssh root@165.154.254.99 "cd /opt/group-ai && source venv/bin/activate && pip install -r requirements.txt"
```

## 步驟5: 創建配置文件

```powershell
ssh root@165.154.254.99 "cat > /opt/group-ai/.env << 'EOF'
# 應用配置
APP_NAME=Group AI Worker Node
APP_ENV=production
DEBUG=false
NODE_ID=worker-01
NODE_ROLE=worker

# 數據庫配置
DATABASE_URL=sqlite:////opt/group-ai/data/group_ai.db

# Telegram API配置（需要修改）
TELEGRAM_API_ID=your_api_id
TELEGRAM_API_HASH=your_api_hash

# OpenAI配置（需要修改）
OPENAI_API_KEY=your_openai_api_key
OPENAI_MODEL=gpt-3.5-turbo

# Session文件配置
SESSION_FILES_DIRECTORY=/opt/group-ai/sessions
SESSION_FILES_BACKUP_DIRECTORY=/opt/group-ai/sessions_backup

# 帳號配置
MAX_ACCOUNTS_PER_NODE=5
ACCOUNT_HEALTH_CHECK_INTERVAL=60
ACCOUNT_RECONNECT_DELAY=5
ACCOUNT_MAX_RECONNECT_ATTEMPTS=3

# 日誌配置
LOG_LEVEL=INFO
LOG_FILE=/opt/group-ai/logs/worker.log
LOG_MAX_SIZE=100MB
LOG_BACKUP_COUNT=10

# 監控配置
METRICS_ENABLED=true
METRICS_PORT=9091
HEARTBEAT_INTERVAL=30
EOF"
```

## 步驟6: 創建config.py

```powershell
ssh root@165.154.254.99 "cat > /opt/group-ai/config.py << 'EOF'
import os
from dotenv import load_dotenv

load_dotenv()

# Telegram API配置
API_ID = int(os.getenv('TELEGRAM_API_ID', '0'))
API_HASH = os.getenv('TELEGRAM_API_HASH', '')

# OpenAI配置
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY', '')
OPENAI_MODEL = os.getenv('OPENAI_MODEL', 'gpt-3.5-turbo')
EOF"
```

## 步驟7: 配置API密鑰

編輯配置文件並填入正確的API密鑰：

```powershell
ssh root@165.154.254.99
# 然後在服務器上執行:
vi /opt/group-ai/.env
```

需要修改：
- `TELEGRAM_API_ID`: 從 https://my.telegram.org 獲取
- `TELEGRAM_API_HASH`: 從 https://my.telegram.org 獲取
- `OPENAI_API_KEY`: 您的OpenAI API Key（如果使用）

## 步驟8: 上傳Session文件

```powershell
# 上傳Session文件
scp *.session root@165.154.254.99:/opt/group-ai/sessions/
```

## 步驟9: 創建啟動腳本

```powershell
ssh root@165.154.254.99 "cat > /opt/group-ai/start.sh << 'EOF'
#!/bin/bash
cd /opt/group-ai
source venv/bin/activate
export PYTHONPATH=/opt/group-ai:\$PYTHONPATH

# 如果存在admin-backend目錄，使用uvicorn啟動
if [ -d \"admin-backend\" ]; then
    cd admin-backend
    python -m uvicorn app.main:app --host 0.0.0.0 --port 8000
else
    # 否則啟動group_ai_service
    python -m group_ai_service.main --host 0.0.0.0 --port 8000
fi
EOF
chmod +x /opt/group-ai/start.sh"
```

## 步驟10: 創建systemd服務

```powershell
ssh root@165.154.254.99 "cat > /etc/systemd/system/group-ai-worker.service << 'EOF'
[Unit]
Description=Group AI Worker Node Service
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/group-ai
Environment=\"PATH=/opt/group-ai/venv/bin\"
Environment=\"PYTHONPATH=/opt/group-ai\"
ExecStart=/opt/group-ai/venv/bin/python -m uvicorn app.main:app --host 0.0.0.0 --port 8000
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
EOF
systemctl daemon-reload
systemctl enable group-ai-worker
systemctl start group-ai-worker
systemctl status group-ai-worker"
```

## 步驟11: 驗證部署

```powershell
# 檢查服務狀態
ssh root@165.154.254.99 "systemctl status group-ai-worker"

# 檢查端口
ssh root@165.154.254.99 "netstat -tlnp | grep 8000"

# 測試API
curl http://165.154.254.99:8000/health

# 查看日誌
ssh root@165.154.254.99 "tail -f /opt/group-ai/logs/worker.log"
```

## 快速執行所有步驟

如果您想一次性執行所有步驟，可以使用以下PowerShell腳本：

```powershell
# 注意：每個SSH命令都需要輸入密碼: Along2025!!!

# 步驟1: 測試連接
ssh-keygen -R 165.154.254.99
ssh -o StrictHostKeyChecking=accept-new root@165.154.254.99 "echo '連接成功'"

# 步驟2: 執行基礎部署
.\scripts\deployment\deploy_remote_interactive.ps1

# 然後按照提示繼續執行後續步驟...
```

## 注意事項

1. **每次SSH連接都需要輸入密碼**: `Along2025!!!`
2. **如果連接失敗**: 檢查服務器是否運行，防火牆是否開放22端口
3. **如果命令執行失敗**: 查看錯誤信息，可能需要手動執行某些步驟
4. **建議配置SSH密鑰**: 部署完成後，建議配置SSH密鑰認證，避免每次輸入密碼

## 需要幫助？

如果遇到問題，請查看：
- [遠程服務器快速部署指南](../docs/部署方案/遠程服務器快速部署指南.md)
- [分散式部署方案](../docs/部署方案/分散式部署方案.md)

