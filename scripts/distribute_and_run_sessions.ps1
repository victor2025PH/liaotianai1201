# 智能分发 Session 文件到各服务器并启动多个账号
$ErrorActionPreference = "Continue"
Import-Module Posh-SSH

Write-Host "`n============================================================" -ForegroundColor Cyan
Write-Host "  智能分发 Session 文件并启动多个账号" -ForegroundColor Cyan
...
foreach ($server in $servers) {
    ...
    try {
        ...
        # 3.1 准备服务器环境
        Write-Host "3.1 准备服务器环境..." -ForegroundColor Cyan
        $r1 = Invoke-SSHCommand -SessionId $sshSession.SessionId -Command "mkdir -p /home/ubuntu/sessions && mkdir -p /home/ubuntu/logs && echo OK"
        Write-Host "  ✓ 目录已准备" -ForegroundColor Green
        Write-Host ""
        
        # 3.2 停止所有旧的 main.py 进程
        Write-Host "3.2 停止所有旧的 main.py 进程..." -ForegroundColor Cyan
        $r2 = Invoke-SSHCommand -SessionId $sshSession.SessionId -Command "pkill -f 'python.*main.py'; sleep 2; echo 'stopped'"
        ...
        foreach ($sessionFile in $serverSessions[$server.IP]) {
            ...
            try {
                # 使用 SCP 上传
                $scp = Set-SCPFile -ComputerName $server.IP -Credential $cred -LocalFile $localPath -RemotePath $remotePath -ErrorAction Stop
                Write-Host "    ✓ 上传成功" -ForegroundColor Green
                $uploadedSessions += @{
                    Local = $localPath
                    Remote = $remotePath
                    Name = $sessionFile.BaseName
                }
            } catch {
                Write-Host "    ✗ 上传失败: $($_.Exception.Message)" -ForegroundColor Red
                # 尝试使用 base64 编码传输
                Write-Host "    尝试使用 base64 传输..." -ForegroundColor Yellow
...
"@
                ...
                if ($r3.Output -match "OK") {
                    Write-Host "    ✓ 上传成功（base64）" -ForegroundColor Green
                    $uploadedSessions += @{
                        Local = $localPath
                        Remote = $remotePath
                        Name = $sessionFile.BaseName
        ...
        # 3.4 为每个 Session 启动独立的 main.py 进程
...
export TELEGRAM_SESSION_FILE='$($sess.Remote)'
...
"@
        ...
        # 3.5 等待并检查进程状态
        Write-Host "3.5 检查进程状态（等待 15 秒）..." -ForegroundColor Cyan
        ...
        foreach ($sess in $uploadedSessions) {
            ...
            $r7 = Invoke-SSHCommand -SessionId $sshSession.SessionId -Command "tail -10 $logFile 2>/dev/null | grep -i '驗證成功\|Session 驗證成功\|connected\|started\|ready\|已啟動\|TG机器人\|Enter phone\|EOFError' | tail -3"
            if ($r7.Output) {
                if ($r7.Output -match "驗證成功|Session 驗證成功|connected|started|ready|已啟動|TG机器人") {
                    ...
                    Write-Host "  ⚠ $($sess.Name): 状态不明确" -ForegroundColor Yellow
                }
            } else {
