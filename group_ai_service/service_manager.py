...
class ServiceManager:
    ...
    def get_script(self, script_id: str, yaml_content: Optional[str] = None) -> Optional[Script]:
        ...
        # 先從緩存獲取（僅在沒有提供新內容時）
        elif script_id in self._scripts_cache:
        ...
        # 嘗試多個可能的路徑
        possible_paths = [
            Path(f"ai_models/group_scripts/{script_id}.yaml"),  # 當前工作目錄
            Path(__file__).parent.parent / f"ai_models/group_scripts/{script_id}.yaml",  # 項目根目錄（從service_manager.py位置推斷）
        ]
        
        # 如果配置中有 scripts_directory，也嘗試使用
        try:
            from group_ai_service.config import get_group_ai_config
            config = get_group_ai_config()
            if config.scripts_directory:
                possible_paths.append(Path(config.scripts_directory) / f"{script_id}.yaml")
        except:
            pass
        ...
        for script_path in possible_paths:
            if script_path.exists():
                try:
                    ...
                    self._scripts_cache[script_id] = script
                    logger.info(f"從文件加載劇本: {script_id} (路徑: {script_path})")
                    return script
                except Exception as e:
                    logger.error(f"從文件加載劇本失敗 ({script_id}, 路徑: {script_path}): {e}")
                    continue
        
        logger.warning(f"劇本 {script_id} 未找到（已嘗試路徑: {[str(p) for p in possible_paths]}）")
        return None
