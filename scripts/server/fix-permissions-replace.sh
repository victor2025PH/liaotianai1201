#!/bin/bash
# 完全替换 permissions.py 为正确版本

echo "修复 permissions.py 语法错误（完全替换）..."

cd ~/telegram-ai-system/admin-backend || exit 1

# 备份
cp app/core/permissions.py app/core/permissions.py.bak.$(date +%Y%m%d_%H%M%S)

# 创建正确的 permissions.py 文件
cat > app/core/permissions.py << 'PERMISSIONS_EOF'
"""
權限定義 - 系統所有權限的集中定義
"""
...
if [ $? -ne 0 ]; then
    ...
    cp app/core/permissions.py.bak.* app/core/permissions.py 2>/dev/null || true
    exit 1
fi

# 测试导入
echo "测试导入 PermissionCode..."
python3 -c "import sys; sys.path.insert(0, '.'); from app.core.permissions import PermissionCode, PERMISSION_GROUPS; print('✅ GROUP_AI_MANAGE =', PermissionCode.GROUP_AI_MANAGE); print('✅ 权限组数量:', len(PERMISSION_GROUPS))" 2>&1
